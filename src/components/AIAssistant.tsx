...
export const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  const processCommand = (text: string) => {
    ...
    let responseText =
      "I didn't understand that command. Try 'Update name to [Name]' or 'Add skill [Skill]'.";
...
• Mentored junior team members and fostered a culture of technical excellence.`;
    ...
    // Add Skill
      ...
      lowerText.includes("add skill") ||
      lowerText.includes("add skills")
    ) {
      const newSkills = text
        .replace(/.*add skills?\s+/i, "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    // Clear Skills
      ...
      lowerText.includes("clear skills") ||
      lowerText.includes("remove all skills")
    ) {
    ...
    // Update Summary
      ...
      lowerText.includes("summary to") ||
      lowerText.includes("change summary")
    ) {
      const summary = text
        .replace(/.*(summary to|change summary)\s+/i, "")
        .trim();
      if (summary) {
            ...
            {messages.map((msg) => (
              <div
                ...
                style={{
                  ...
                  boxShadow:
                    msg.sender === "ai" ? "0 2px 8px rgba(0,0,0,0.05)" : "none",
                  fontSize: "14px",
                  lineHeight: "1.4",
